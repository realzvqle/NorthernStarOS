...
static void qemu_cfg_dma_transfer(void *address, uint32_t length, uint32_t control) {
    QemuCfgDmaAccess access = {.control = bswap32(control), .length = bswap32(length),   .address = bswap64((uint64_t)address) };
...
int qemu_cfg_find_file() {
    ...
    for (select = 0, e = 0; e < count; e++) {
        ...
        if (memcmp_str(qfile.name, (void*)"etc/ramfb", 10) == 0)
            select = bswap16(qfile.select);
    }
    return select;
}
